...
export async function sendApplyToBff(
  ...
  // Cities the applicant kept featured placement for (this schema tracks a
  // single "featuredPlacement" toggle plus an opt-out list per city, rather
  // than an explicit list of featured cities like the reference site).
  const featuredCities = data.featuredPlacement
    ? data.locations.filter((l) => !data.excludedFeatured.includes(`${l.city}|${l.state}`))
    : [];
  ...
  const payload = {
    ...
    // This schema has no executives/key-staff field; default to empty array.
    key_staff: [] as string[],
